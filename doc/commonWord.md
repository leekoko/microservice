...
自动部署，当推镜像的版本发生变化，就会进行升级

把生成的内容挂载出来（宿主机），不然再启动就会显示   

有关联的容器，主机目录，pod里可以共享。
...
配置文件，配置里可以进行配置，然后服务中进行选用。可以替换配置文件   
...
启动命令可以覆盖镜像里面的命令，命令和参数进行分离   



容器大小       0,0（全部置0）   让其自己去适用    
